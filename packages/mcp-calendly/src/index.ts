#!/usr/bin/env node
/**
 * @bridgeapi/mcp-calendly — MCP Server
...
function loadConfig(): CalendlyConfig {
  ...
  if (!calendly_access_token) throw new Error("CALENDLY_ACCESS_TOKEN is required");
  return {
...
async function executeWithHooks<T>(
  toolName: string, params: Record<string, unknown>, config: CalendlyConfig,
...
const server = new McpServer(
  { name: "bridgeapi-calendly", version: "0.1.0" },
...
// ═══ RESOURCES ═══
server.resource("items", "calendly://items", async () => {
  const result = await client.request("/items?limit=50");
  return { contents: [{ uri: "calendly://items", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("orders", "calendly://orders", async () => {
  const result = await client.request("/orders?limit=50");
  return { contents: [{ uri: "calendly://orders", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("profile", "calendly://profile", async () => {
  const result = await client.request("/me");
  return { contents: [{ uri: "calendly://profile", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
