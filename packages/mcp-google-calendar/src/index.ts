...
function loadConfig(): GoogleCalendarConfig {
  ...
  if (!google_refresh_token) throw new Error("GOOGLE_REFRESH_TOKEN is required");
  return {
...
async function executeWithHooks<T>(
  toolName: string, params: Record<string, unknown>, config: GoogleCalendarConfig,
...
// ═══ RESOURCES ═══
server.resource("items", "gcal://items", async () => {
  const result = await client.request("/items?limit=50");
  return { contents: [{ uri: "gcal://items", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("orders", "gcal://orders", async () => {
  const result = await client.request("/orders?limit=50");
  return { contents: [{ uri: "gcal://orders", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("profile", "gcal://profile", async () => {
  const result = await client.request("/me");
  return { contents: [{ uri: "gcal://profile", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
...
async function main() {
  ...
  console.error("BridgeAPI Google Calendar MCP Server v0.1.0 running on stdio");
}
