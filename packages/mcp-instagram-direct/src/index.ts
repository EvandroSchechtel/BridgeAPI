...
function loadConfig(): InstagramConfig {
  ...
  if (!instagram_access_token) throw new Error("INSTAGRAM_ACCESS_TOKEN is required");
  const instagram_page_id = process.env.INSTAGRAM_PAGE_ID;
  if (!instagram_page_id) throw new Error("INSTAGRAM_PAGE_ID is required");
  return {
...
async function executeWithHooks<T>(
  toolName: string, params: Record<string, unknown>, config: InstagramConfig,
...
// ═══ RESOURCES ═══
server.resource("items", "instagram://items", async () => {
  const result = await client.request("/items?limit=50");
  return { contents: [{ uri: "instagram://items", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("orders", "instagram://orders", async () => {
  const result = await client.request("/orders?limit=50");
  return { contents: [{ uri: "instagram://orders", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("profile", "instagram://profile", async () => {
  const result = await client.request("/me");
  return { contents: [{ uri: "instagram://profile", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
