...
**MCP Server para Loja Integrada API** — Conecte qualquer AI Agent ao Loja Integrada via Model Context Protocol.
...
| Resource | URI |
|----------|-----|
| Itens | `loja-integrada://items` |
| Pedidos | `loja-integrada://orders` |
| Perfil | `loja-integrada://profile` |
...
```bash
npm install @bridgeapi/mcp-loja-integrada
```
...
```json
{
  "mcpServers": {
    "loja-integrada": {
      "command": "npx",
      "args": ["@bridgeapi/mcp-loja-integrada"],
      "env": {}
    }
  }
}
...
```bash
git clone https://github.com/EvandroSchechtel/mcp-loja-integrada.git
cd mcp-loja-integrada
npm install
npm run build
npm run inspect
```
