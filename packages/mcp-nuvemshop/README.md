...
**MCP Server para Nuvemshop API** — Conecte qualquer AI Agent ao Nuvemshop via Model Context Protocol.
...
| Resource | URI |
|----------|-----|
| Itens | `nuvemshop://items` |
| Pedidos | `nuvemshop://orders` |
| Perfil | `nuvemshop://profile` |
...
```bash
npm install @bridgeapi/mcp-nuvemshop
```
...
```bash
git clone https://github.com/EvandroSchechtel/mcp-nuvemshop.git
cd mcp-nuvemshop
npm install
npm run build
npm run inspect
