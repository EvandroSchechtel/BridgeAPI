#!/usr/bin/env node
/**
 * @bridgeapi/mcp-nuvemshop — MCP Server
...
function loadConfig(): NuvemshopConfig {
  ...
  if (!nuvemshop_access_token) throw new Error("NUVEMSHOP_ACCESS_TOKEN is required");
  const nuvemshop_user_id = process.env.NUVEMSHOP_USER_ID;
  if (!nuvemshop_user_id) throw new Error("NUVEMSHOP_USER_ID is required");
  return {
...
async function executeWithHooks<T>(
  toolName: string, params: Record<string, unknown>, config: NuvemshopConfig,
...
const server = new McpServer(
  { name: "bridgeapi-nuvemshop", version: "0.1.0" },
...
// ═══ RESOURCES ═══
server.resource("items", "nuvemshop://items", async () => {
  const result = await client.request("/items?limit=50");
  return { contents: [{ uri: "nuvemshop://items", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("orders", "nuvemshop://orders", async () => {
  const result = await client.request("/orders?limit=50");
  return { contents: [{ uri: "nuvemshop://orders", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("profile", "nuvemshop://profile", async () => {
  const result = await client.request("/me");
  return { contents: [{ uri: "nuvemshop://profile", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
