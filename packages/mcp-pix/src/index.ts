...
function loadConfig(): PixConfig {
  ...
  if (!pix_client_id) throw new Error("PIX_CLIENT_ID is required");
  const pix_client_secret = process.env.PIX_CLIENT_SECRET;
  if (!pix_client_secret) throw new Error("PIX_CLIENT_SECRET is required");
...
async function executeWithHooks<T>(
  toolName: string, params: Record<string, unknown>, config: PixConfig,
...
// ═══ RESOURCES ═══
server.resource("items", "pix://items", async () => {
  const result = await client.request("/items?limit=50");
  return { contents: [{ uri: "pix://items", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("orders", "pix://orders", async () => {
  const result = await client.request("/orders?limit=50");
  return { contents: [{ uri: "pix://orders", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("profile", "pix://profile", async () => {
  const result = await client.request("/me");
  return { contents: [{ uri: "pix://profile", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
