#!/usr/bin/env node
/**
 * @bridgeapi/mcp-shopee — MCP Server
...
function loadConfig(): ShopeeConfig {
  ...
  if (!shopee_partner_id) throw new Error("SHOPEE_PARTNER_ID is required");
  const shopee_partner_key = process.env.SHOPEE_PARTNER_KEY;
  if (!shopee_partner_key) throw new Error("SHOPEE_PARTNER_KEY is required");
  const shopee_access_token = process.env.SHOPEE_ACCESS_TOKEN;
  if (!shopee_access_token) throw new Error("SHOPEE_ACCESS_TOKEN is required");
  return {
...
async function executeWithHooks<T>(
  toolName: string, params: Record<string, unknown>, config: ShopeeConfig,
...
// ═══ RESOURCES ═══
server.resource("items", "shopee://items", async () => {
  const result = await client.request("/items?limit=50");
  return { contents: [{ uri: "shopee://items", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("orders", "shopee://orders", async () => {
  const result = await client.request("/orders?limit=50");
  return { contents: [{ uri: "shopee://orders", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("profile", "shopee://profile", async () => {
  const result = await client.request("/me");
  return { contents: [{ uri: "shopee://profile", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
