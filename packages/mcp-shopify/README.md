...
**MCP Server para Shopify API** — Conecte qualquer AI Agent ao Shopify via Model Context Protocol.
...
|----------|-----|
| Itens | `shopify://items` |
| Pedidos | `shopify://orders` |
| Perfil | `shopify://profile` |
...
```bash
npm install @bridgeapi/mcp-shopify
```
...
```bash
git clone https://github.com/EvandroSchechtel/mcp-shopify.git
cd mcp-shopify
npm install
npm run build
npm run inspect
