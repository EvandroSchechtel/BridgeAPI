#!/usr/bin/env node
/**
 * @bridgeapi/mcp-shopify — MCP Server
...
function loadConfig(): ShopifyConfig {
  ...
  if (!shopify_store_url) throw new Error("SHOPIFY_STORE_URL is required");
  const shopify_access_token = process.env.SHOPIFY_ACCESS_TOKEN;
  if (!shopify_access_token) throw new Error("SHOPIFY_ACCESS_TOKEN is required");
  return {
...
async function executeWithHooks<T>(
  toolName: string, params: Record<string, unknown>, config: ShopifyConfig,
...
const server = new McpServer(
  { name: "bridgeapi-shopify", version: "0.1.0" },
...
// ═══ RESOURCES ═══
server.resource("items", "shopify://items", async () => {
  const result = await client.request("/items?limit=50");
  return { contents: [{ uri: "shopify://items", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("orders", "shopify://orders", async () => {
  const result = await client.request("/orders?limit=50");
  return { contents: [{ uri: "shopify://orders", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
server.resource("profile", "shopify://profile", async () => {
  const result = await client.request("/me");
  return { contents: [{ uri: "shopify://profile", mimeType: "application/json", text: JSON.stringify(result.data, null, 2) }] };
});
